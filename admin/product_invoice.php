<?php
/**
 * Admin - Product Approval Invoice
...
// Get product details (only if approved)
$stmt = $conn->prepare("
    ...
    WHERE fp.product_id = ? AND fp.status = 'approved'
");
if ($stmt === false) {
...
$salesStmt = $conn->prepare("
    ...
    WHERE oi.product_id = ? AND co.status != 'Cancelled'
");
...
$totalQuantitySold = $salesData['total_quantity_sold'] ?? 0;
$totalRevenue = $salesData['total_revenue'] ?? 0;
$firstSaleDate = $salesData['first_sale_date'];
$lastSaleDate = $salesData['last_sale_date'];

// Get recent sales for this product
$recentSalesStmt = $conn->prepare("
    SELECT oi.*, co.order_date, co.status,
    ...
    LIMIT 5
");
$recentSalesStmt->bind_param("i", $productId);
$recentSalesStmt->execute();
$recentSales = $recentSalesStmt->get_result()->fetch_all(MYSQLI_ASSOC);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Product Listing Invoice #<?php echo $invoiceNumber; ?> | AgriSphere Admin</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        .sales-stat-box.highlight .sales-stat-label {
    ...
    <div class="invoice">
        ...
        <div class="header">
            ...
            <div class="invoice-info">
                ...
                <span class="admin-badge">Admin Copy</span>
        ...
        <div class="farmer-info">
            <div>
                <h4>Seller (Farmer)</h4>
                <p><strong><?php echo sanitize($product['First_name'] . ' ' . $product['Last_name']); ?></strong></p>
                <p><?php echo sanitize($product['Email']); ?></p>
                <p><?php echo sanitize($product['Phone'] ?? ''); ?></p>
            </div>
            <div style="text-align: right;">
                <h4>Farmer ID</h4>
                <p><strong>#<?php echo $product['farmer_id']; ?></strong></p>
                <p><?php echo nl2br(sanitize($product['farmer_address'] ?? '')); ?></p>
            </div>
        </div>
        ...
        <div class="approval-section">
            ...
            <div class="approval-text">This product is now live on AgriSphere marketplace and visible to all customers.</div>
        ...
        <div class="sales-section">
            <div class="sales-header">
                <h3><span class="sales-icon">&#128200;</span> Sales Performance</h3>
            </div>

            <div class="sales-stats">
                <div class="sales-stat-box">
                    <div class="sales-stat-icon orders">&#128722;</div>
                    <div class="sales-stat-value"><?php echo number_format($totalOrders); ?></div>
                    <div class="sales-stat-label">Total Orders</div>
                </div>
                <div class="sales-stat-box">
                    <div class="sales-stat-icon quantity">&#128230;</div>
                    <div class="sales-stat-value"><?php echo number_format($totalQuantitySold); ?></div>
                    <div class="sales-stat-label"><?php echo sanitize($product['unit']); ?> Sold</div>
                </div>
                <div class="sales-stat-box highlight">
                    <div class="sales-stat-icon revenue">&#128176;</div>
                    <div class="sales-stat-value"><?php echo formatCurrency($totalRevenue); ?></div>
            ...
            <div class="sales-period">
                <span>First Sale: <strong><?php echo formatDate($firstSaleDate, 'd M Y'); ?></strong></span>
                <span class="separator">|</span>
                <span>Last Sale: <strong><?php echo formatDate($lastSaleDate, 'd M Y'); ?></strong></span>
            </div>
            <?php endif; ?>
                ...
                <table class="sales-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?php echo formatDate($sale['order_date'], 'd M Y'); ?></td>
                            <td><?php echo $sale['quantity']; ?> <?php echo sanitize($product['unit']); ?></td>
                            <td><?php echo formatCurrency($sale['quantity'] * $sale['price_per_unit']); ?></td>
                            <td>
                                <span class="status-pill <?php echo strtolower($sale['status']); ?>">
            ...
            <div class="no-sales">
                <span class="no-sales-icon">&#128722;</span>
                <p>No sales recorded yet for this product.</p>
            </div>
            <?php endif; ?>
        </div>
        ...
        <!-- Footer -->
        <div class="footer">
            <p><strong>AgriSphere - Admin Product Listing Record</strong></p>
            <p>From Farm to Table, Simplified</p>
