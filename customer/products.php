...
// Browse / search is served by the C++ CatalogService (/api/catalog/products);
// the filter, search term and sort key are applied there.
$productsResponse = AgriSphereBackend::tryCall('/api/catalog/products', [
...
// Get categories for filter (C++ backend: /api/catalog/categories)
...
<?php else: ?>
    <div class="products-grid">
        <?php foreach ($products as $product):
            ...
            $inStock = $stock > 0;
            // Discount info comes from the C++ backend. price_per_unit is
            // always the farmer's list price; effective_price is what you pay.
            $hasDiscount   = (int)($product['has_discount'] ?? 0) === 1;
            $discountLabel = rtrim(rtrim(number_format((float)($product['discount_percent'] ?? 0), 2), '0'), '.');
            $payPrice      = $hasDiscount ? (float)$product['effective_price'] : (float)$product['price_per_unit'];
        ?>
            <div class="product-card">
                <div class="product-image">
                    ...
                    <?php if ($hasDiscount): ?>
                        <span class="product-discount-badge"><?php echo $discountLabel; ?>% OFF</span>
                    <?php endif; ?>
                    <?php if (!$inStock): ?>
                ...
                <div class="product-info">
                    ...
                    <div class="product-meta">
                        <?php if ($hasDiscount): ?>
                            <span class="product-price-old"><?php echo formatCurrency($product['price_per_unit']); ?></span>
                        <?php endif; ?>
                        <span class="product-price"><?php echo formatCurrency($payPrice); ?></span>
...
/* Discount styling (Observer feature) */
