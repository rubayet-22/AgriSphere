...
// Subscription state, so the banner below shows the real expiry date
require_once __DIR__ . '/../includes/farmer_subscription.php';
$subscription = farmerSubscriptionStatus($conn, $farmerId);
...
<?php
// Driven by farmer_subscription, so the state and the expiry date are real.
$subActive  = !empty($subscription['active']);
$subExpires = $subscription['expires_at'] ?? '';
?>
<div class="subscription-banner<?php echo $subActive ? '' : ' subscription-banner-warn'; ?>">
    <div class="subscription-info">
        <div class="subscription-icon">
            <i class="fas <?php echo $subActive ? 'fa-check-circle' : 'fa-exclamation-circle'; ?>"></i>
        </div>
        <div class="subscription-details">
            <?php if ($subActive): ?>
                <h4>Active Subscription</h4>
                <p>
                    You can sell products on AgriSphere until
                    <strong><?php echo formatDateTime($subExpires); ?></strong>.
                </p>
            <?php elseif (!empty($subscription['missing'])): ?>
                <h4>Subscription Not Set Up</h4>
                <p>Run <code>database/farmer_subscription.sql</code> to enable farmer subscriptions.</p>
            <?php else: ?>
                <h4>Subscription Expired</h4>
                <p>
                    Your subscription ended on
                    <strong><?php echo formatDateTime($subExpires); ?></strong>. Renew to keep selling.
                </p>
            <?php endif; ?>
        </div>
    </div>
    <div class="subscription-amount">
        <span class="amount"><?php echo formatCurrency(FARMER_SUBSCRIPTION_FEE); ?></span>
        <span class="period">/ <?php echo FARMER_SUBSCRIPTION_MONTHS; ?> months</span>
        <div style="margin-top: 10px;">
            <a href="<?php echo BASE_URL; ?>farmer/subscription.php"
               class="btn btn-sm <?php echo $subActive ? 'btn-outline' : 'btn-primary'; ?>">
                <i class="fas fa-sync-alt"></i>
                <?php echo $subActive ? 'Renew' : 'Pay Now'; ?>
            </a>
        </div>
    </div>
</div>
...
.subscription-banner-warn .subscription-icon {
